...
//improve the offensive disposition of AI
public class AI_2 
{
	private int x,y;
		...
		for(int i=0;i<19;i++)
		{
			for(int j=0;j<19;j++)
			{
				if(spot[i][j]==0)// is it a blank spot?
				{
					...
					{
						if(LineLength[i][j][k][0]+LineLength[i][j][k+4][0]>=7)
							score+=100000000;
						else if(LineLength[i][j][k][0]+LineLength[i][j][k+4][0]==6)
							score+=1000000;
						else if(LineLength[i][j][k][0]+LineLength[i][j][k+4][0]==5)
							score+=100000;
